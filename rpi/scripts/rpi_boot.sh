...
start_screen rausch localtunnel "lt --port 8000 --subdomain jflytest" server "(cd /home/pi/strike_imp/rpi; (cd web-interface && npm install && npm run build); (cd server && npm install && npm start))"
